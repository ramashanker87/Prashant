...
## Execute cloudformation to create ECS
    ...
    aws cloudformation deploy --template-file ecs-farget-api-gateway.yml --stack-name prashant-create-ecs-farget --capabilities CAPABILITY_NAMED_IAM 
...
### List cluster service
    ...
    aws ecs list-services --cluster prashant-test-calculator-cluster  

### Describe services

    aws ecs describe-services --cluster <CLUSTER_NAME> --services <SERVICE_NAME> --profile prashant

### List Running Tasks

    aws ecs list-tasks --cluster <CLUSTER_NAME> --desired-status RUNNING

    aws ecs list-tasks --cluster <CLUSTER_NAME> --service-name <SERVICE_NAME> --profile prashant
...
### ALB command  to describe LB
    ...
    aws elbv2 describe-load-balancers --profile prashant
